...
ELClientPacket * temp(void* punt, uint16_t len)
{
	uint16_t* p2 = (uint16_t*) punt;
	p2 --;
	p2 --;
	ELClientPacket * packet = (ELClientPacket *) p2;
	if(len > 0)
	{
		packet->cmd = CMD_SOCKET_SEND;
		packet->argc = len;
...
ELClientPacket *ELClient::Process() {
	...
	/* se mi aspettavo un pacchetto, provo a decodificarlo.
	 * Se la decodifica fallisce, restituisci tutto
	 * Se la decodifica ha successo, allora mi aspetto un non-pacchetto
	 * Se non mi aspetto un pacchetto, restituisci tutto e aspetta il pacchetto*/
  ...
  while (_serial->available()) {
    ...
    if (value == SLIP_ESC) {
      _proto.isEsc = 1;
		...
		//printf("slend: %u %u\n",_proto.dataLen,_proto.isPacket);
	  if(_proto.isPacket == 0)
	  {
		  
		  ELClientPacket *packet = NULL;
		  if(_proto.dataLen > 0)
		  {
			packet = temp(_proto.buf,_proto.dataLen);
		  }
		  _proto.isPacket = 1;
		  _proto.dataLen = 0;
		  _proto.isEsc = 0;
  ...
  if(_proto.isPacket == 0 && _proto.bufSize > 0)
  {
	  ELClientPacket *packet = temp(_proto.buf,_proto.dataLen);
	  _proto.dataLen = 0;
	  _proto.isEsc = 0;
	  return packet;
  }
...
*/
void ELClient::init() {
  _proto.buf = _protoBuf + (2*sizeof(uint16_t));
  _proto.bufSize = sizeof(_protoBuf) - (2*sizeof(uint16_t));
  _proto.dataLen = 0;
  _proto.isEsc = 0;
  _proto.isPacket=1;
}
...
ELClientPacket *ELClient::WaitReturn(uint32_t timeout) {
  ...
  while (milliseconds() - wait < timeout) {
...
void ELClient::SyncStart()
{
  ...
  resetCbBak = resetCb;
  resetCb = NULL;
  syncStartMs = milliseconds();
	
}
enum SyncResult  ELClient::SyncCheck(uint16_t timeout)
{
	...
	uint16_t diff = (milliseconds() - syncStartMs);
	if(diff < timeout)
	{
		return SYNC_RESULT_RUNNING;
	}
	
  // doesn't look like we got a real response
  resetCb = resetCbBak;
  return SYNC_RESULT_FAILURE;
}
...
boolean ELClient::Sync(uint32_t timeout) {
	...
	while((result = SyncCheck(timeout) ) == SYNC_RESULT_RUNNING)
	{
		;
	}
